...
func (s *DTEService) ProcessNotaDebito(ctx context.Context, nota *models.NotaDebito) (*hacienda.ReceptionResponse, error) {
	...
	if err != nil {
		...
		if s.contingencyHelperNota != nil {
			if queueErr := s.contingencyHelperNota.HandleNotaDebitoSigningFailure(
				ctx,
				nota,
				dteForSigning,
				"00", // ambiente - TODO: get from company
			); queueErr != nil {
				return nil, fmt.Errorf("firmador failed and contingency queue failed: %w", queueErr)
			}
			fmt.Println("📋 Nota Débito queued for contingency (firmador unavailable)")
			return nil, fmt.Errorf("nota debito queued for contingency: firmador unavailable")
		}
	...
	if err != nil {
		...
		if s.contingencyHelperNota != nil {
			if queueErr := s.contingencyHelperNota.HandleNotaDebitoAuthFailure(
				ctx,
				nota,
				dteForSigning,
				signedDTE,
			...
			fmt.Println("📋 Nota Débito queued for contingency (Hacienda auth unavailable)")
			return nil, fmt.Errorf("nota debito queued for contingency: Hacienda auth unavailable")
		}
	...
	if err != nil {
		...
		if s.contingencyHelperNota != nil {
			if queueErr := s.contingencyHelperNota.HandleNotaDebitoSubmissionFailure(
				ctx,
				nota,
				dteForSigning,
				signedDTE,
			...
			fmt.Println("📋 Nota Débito queued for contingency (Hacienda unavailable)")
			return nil, fmt.Errorf("nota debito queued for contingency: Hacienda unavailable")
		}
...
func (s *DTEService) _ProcessNotaDebito(ctx context.Context, nota *models.NotaDebito) (*hacienda.ReceptionResponse, error) {
	// Step 1: Build DTE from nota
	fmt.Printf("\n🔄 Processing Nota de Débito: %s\n", nota.NotaNumber)
	fmt.Println("Step 1: Building DTE from nota de débito...")
